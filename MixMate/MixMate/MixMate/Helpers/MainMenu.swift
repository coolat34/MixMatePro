...
@Published var bagWeight: [String] = ["20kg", "25kg", "30kg"]
...
// Update the notationAbr & Bags if the Measurement changes
    ...
    func updateRegion(for system: String) {
        if system == "metric" {
            ...
            bagWeight = ["20kg", "25kg", "30kg"]
        } else {
            measureSelected = "ft"
            note = "ft"
            measurements = ["Inches", "Feet"]
            notationAbr = ["in", "ft"]
            bagWeight = ["40lb", "60lb", "80lb"]
        }
    }
            ...
            ZStack {
                ...
                VStack {
                    Text("Calculate concrete volume and mix for common shapes. Choose measurements, select strength, and get bag estimates instantly.")
                        ...
                        .padding()
                //    Text("Country Code \(VM.CountryCode ?? "") ")
        ...
        ZStack {  // Cover the entire screen
            ...
            VStack {
                VStack {
                 ...
                 //   Add a toggle or picker to amend referred unit
                    ...
                    .onChange(of: preferredUnitSystem) {
                        VM.updateRegion(for: preferredUnitSystem)
                    }
...
struct bagFigs: View {
    ...
    var body: some View {
        ...
        // MARK: Bags Value
        OutputBagField(label: VM.bagWeight[0], valCem: VM.BagsCementSmall, valSand: VM.BagsSandSmall, valAgg: VM.BagsAggregateSmall)
        OutputBagField(label: VM.bagWeight[1], valCem: VM.BagsCementMed, valSand: VM.BagsSandMed, valAgg: VM.BagsAggregateMed)
        OutputBagField(label: VM.bagWeight[2], valCem: VM.BagsCementLarge, valSand: VM.BagsSandLarge, valAgg: VM.BagsAggregateLarge)
